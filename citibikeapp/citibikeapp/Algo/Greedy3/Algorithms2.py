from AlgoObjects import StationMap, Break
...
def daytime_routing(forbidden_stations, prefix_routes, previous_solution, driverless_tasks,
    ...
    for j in xrange(len(trucks)):
        ...
        if shift_end.get(id_number) is None or shift_end[id_number] >= look_ahead_end:
            temp_trucks[j].end = look_ahead_end
        else:
            temp_trucks[j].end = shift_end[id_number]

    # See if you need to allocate time for exiting
    needs_exit = dict.fromkeys([truck.id for truck in trucks])
    for i in shift_end:
        if shift_end[i] is None or temp_trucks[i].end < shift_end[i]:
            needs_exit[i] = False
        else:
            needs_exit[i] = True
    ...
    for truck in temp_trucks:
        ...
        # Truck performs fixed tasks, route is cleared
    ...
    for task in driverless_tasks:
        assign_task = schedule_task(temp_trucks, smap, task)
    ...
    algo_output = grasp_time_best(trucks, smap, runtime - time.time() - start, 5, needs_exit, best_score)
